...
class LIARNeuron(nn.Module):
    ...
    def __init__(
        ...
        self.phase_freq_base = nn.Parameter(torch.ones(out_features))
    ...
    def forward(self, x: torch.Tensor, return_history: bool = False):
        ...
        for step in range(self.max_steps):
            ...
            if self.latent_dim is None:
                ...
                E_tensor = E_tensor / (self.in_features ** 0.5)
            else:
            ...
            if self.max_order >= 3:
                ...
                E_tri = E_tri / float(self.in_features)
            ...
            charge_sum = torch.sum(x_log, dim=1, keepdim=True) # (B, 1)
